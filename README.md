...
1. You can view what branches are available in the repository by typing `git branch --list`. This returns only the *master* branch, as it's the only one we have on our computer. 
   ...
   However, if we enter the command `git log --branches --remotes --graph --oneline --decorate` and look at the red text, there are actually two branches in the remote repository: *master* and *develop*.
